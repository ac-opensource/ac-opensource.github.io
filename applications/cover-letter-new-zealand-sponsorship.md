Andrew V. Concepcion
AI-Native Software Engineer | Kotlin | Jetpack Compose | Mobile Architecture
aarconcepcion@gmail.com | NZ +64 290 431 2382 | PH +63 917 815 4366 | https://ac-opensource.github.io

Dear Hiring Team,

I am writing to express interest in software engineering opportunities in New Zealand where strong product delivery, technical ownership, and mobile depth matter. I am an AI-native software engineer with more than 10 years of experience shipping production systems across crypto, fintech, streaming, telecom, transport, rewards, safety, and e-commerce. My core expertise is in Kotlin, Jetpack Compose, and modern Android architecture, supported by hands-on experience across iOS, backend integrations, shared native layers, release engineering, and cross-functional delivery.

I currently work as an AI-Native Software Engineer at Bitcoin.com. I led the first end-to-end Android implementation of a multichain rewards platform, built reward-backed crypto market predictions, co-built Bitcoin replace-by-fee, and led an in-app single-chain-to-multichain wallet migration. I also modernized staking and liquidity-reward pool reads, shipped provider-aware swaps and recurring-buy reminders, moved services into shared Rust through UniFFI, and hardened startup and Android releases with agent-first workflows.

Before Bitcoin.com, I worked at Candyspace on the ITVX Android media player. I built an in-player recommendations panel with coordinated animations and player-control movement across phone and tablet, independently delivered preview timeline scrubbing, and improved reliability through error tracking, security fixes, and a Simple XML-to-Jackson migration. Earlier, at Red Airship, I built OCBC Business mobile banking flows across onboarding, account dashboards, transfers, and digital account opening, and improved OpenPay checkout and payment reliability. I also led five senior Android engineers on MySTC, serving more than 10 million installs across multilingual customer journeys.

What I bring is a combination of high-output execution and sound engineering judgment. I use AI tools effectively to accelerate investigation and delivery, while grounding decisions in architecture, performance, testing, and evidence. I am comfortable taking technical ownership, mentoring engineers, and helping teams move from roadmap to reliable production outcomes.

My family and I have immediate family in New Zealand who are permanent residents and citizens, which gives us a strong personal foundation for relocation. We are willing to migrate for the right opportunity, and I would welcome employer sponsorship if required.

I am looking for an opportunity where I can contribute as someone who can deliver quickly, work well across functions, and help strengthen both execution speed and engineering quality. I would welcome the opportunity to discuss how my background can support your team.

Thank you for your time and consideration.

Sincerely,

Andrew V. Concepcion
